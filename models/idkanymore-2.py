...
""" 
# Step 5: Model Stacking with Hyperparameter Optimization
# Define base models
def get_models(trial):
    lgb_params = {
        'n_estimators': trial.suggest_int('lgb_n_estimators', 1000, 3000),
        'learning_rate': trial.suggest_float('lgb_learning_rate', 0.005, 0.05),
    ...
    xgb_params = {
        'n_estimators': trial.suggest_int('xgb_n_estimators', 1000, 3000),
        'learning_rate': trial.suggest_float('xgb_learning_rate', 0.005, 0.05),
    ...
    cb_params = {
        'iterations': trial.suggest_int('cb_iterations', 1000, 3000),
        'learning_rate': trial.suggest_float('cb_learning_rate', 0.005, 0.05),
        'depth': trial.suggest_int('cb_depth', 5, 15),
...
def objective(trial):
    estimators = get_models(trial)
    stacking_regressor = StackingRegressor(
        estimators=estimators,
        final_estimator=Ridge(alpha=trial.suggest_float('ridge_alpha', 0.1, 10)),
        cv=5,
        n_jobs=-1
    )
...
estimators = get_models(best_trial)
stacking_regressor = StackingRegressor(
    estimators=estimators,
    final_estimator=Ridge(alpha=best_trial.params['ridge_alpha']),
    cv=5,
    n_jobs=-1
)
